...
class ChildChartScreen extends React.Component {
    ...
    checkParams() {
        const { lastRegistroEstatura, lastRegistroPeso, child } = this.props.route.params;
        ...
        // console.log("child", child.id, uid);
        this.uid = uid;
    ...
    getDataCharts() {
        let userId = this.uid;
        let childId = this.childId;// setChildId(idPesos.id);
        let params = {
            url: "getDataCharts",
            data: { userId, childId }
        }
        ServiceManager.prototype.Post(params)
            .then(response => {
